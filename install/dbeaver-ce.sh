...
API_URL=https://api.github.com/repos/dbeaver/dbeaver/releases/latest
...
if dpkg -s dbeaver-ce &>/dev/null; then
    CURRENT_VERSION=$(dpkg -s dbeaver-ce | grep ^Version: | cut -f2 -d " ")
else
...
if ! [[ "$LATEST_VERSION" == *"$CURRENT_VERSION"* || "$CURRENT_VERSION" == *"$LATEST_VERSION"* ]]; then
    wget -qO- --header="Authorization: Bearer $GITHUB_TOKEN" "$API_URL" | \
        ...
        xargs wget -O "$TEMP_DIR/dbeaver-ce.deb"
    if [[ -z "$(command -v gdebi)" ]]; then
        sudo apt-get update
        sudo apt-get install -y gdebi
    fi
    sudo gdebi -n "$TEMP_DIR/dbeaver-ce.deb"
fi
