...
SOGOUPINYIN_LATEST_VERSION="$(wget --show-progress -qO- https://shurufa.sogou.com/linux | grep -Po "https://ime-sec.*?amd64.deb" | cut -f2 -d "_")"
...
if [[ "$SOGOUPINYIN_LATEST_VERSION" == *"$SOGOUPINYIN_INSTALLED_VERSION"* || "$SOGOUPINYIN_INSTALLED_VERSION" == *"$SOGOUPINYIN_LATEST_VERSION"* ]]; then
    ...
    wget --show-progress -O "$TMPDIR/sogoupinyin.deb" "$(wget --show-progress -qO- https://shurufa.sogou.com/linux | grep -Po "https://ime-sec.*?amd64.deb")"
