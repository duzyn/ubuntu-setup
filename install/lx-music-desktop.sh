...
API_URL=https://api.github.com/repos/lyswhut/lx-music-desktop/releases/latest
LATEST_VERSION=$(wget -qO- --header="Authorization: Bearer $GITHUB_TOKEN" "$API_URL" | grep tag_name | cut -f4 -d "\"" | tr -d "v")

if dpkg -s lx-music-desktop &>/dev/null; then
    CURRENT_VERSION=$(dpkg -s lx-music-desktop | grep ^Version: | cut -f2 -d " ")
else
...
if ! [[ "$LATEST_VERSION" == *"$CURRENT_VERSION"* || "$CURRENT_VERSION" == *"$LATEST_VERSION"* ]]; then
    wget -qO- --header="Authorization: Bearer $GITHUB_TOKEN" "$API_URL" | \
        grep -Po "https://.+x64\.deb" | head -n 1 | \
        sed -e "s|https://github.com|https://ghproxy.com/https://github.com|g" | \
        xargs wget -O "$TEMP_DIR/lx-music-desktop.deb"
    if [[ -z "$(command -v gdebi)" ]]; then
        sudo apt-get update
        sudo apt-get install -y gdebi
    fi
    sudo gdebi -n "$TEMP_DIR/lx-music-desktop.deb"
fi
